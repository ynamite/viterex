...
- [ ] redaxo installer config file (`templates/redaxo/redaxo_install_config.json`) can optionally be provided by the user in `redaxo_installer_config.json` and shouldn't be hard coded. If presented, it should be used in `scaffold-frontend.ts` at `// var/data/addons/install/config.json` to be copied to the data dir (as is the case now). If none is provided, this step should be skipped.
- [ ] redaxo custom sql seed file (`templates/redaxo/redaxo_massif_install.sql.tpl`) is entirely optional and should be provided by the user in `redaxo_custom_sql_seed.sql.tpl` instead of hard coded. It can be used to seed custom data during redaxo installation, like we currently do for the massif demo content. If provided, it should be executed at the same moment it is now in `scaffold-frontend.ts` and also allow usage of replacements like `{{DB_NAME}}`, `{{DB_USER}}`, etc. for dynamic values based on user input.
- [ ] custom configuration like `addon.json`, `redaxo_installer_config.json`, `redaxo_custom_sql_seed.sql.tpl` and `package.json.tpl`
