...
const createSvg = (options) => {
    const { path, seedValue } = generator(options);
    ...
    svg.setAttribute('data-seed', seedValue);
    let html = `<path fill="#e7e7e7" d="${path}" />`;
    svg.innerHTML = html;
    return { svg, seedValue, path };
};
...
class blobshape {
    constructor(elements = [], options = {}) {
        ...
        if (elements.length === 0) {
            const { svg, seedValue, path } = createSvg(this.options);
            return { svg: svg, seed: seedValue, path: path };
        }
        elements =
        ...
        elements.forEach((element) => {
            const { svg, seedValue, path } = createSvg(this.options);
            shapes.push({ svg: svg, seed: seedValue, path: path });
            element.append(svg);
        });
        return shapes;
    }
