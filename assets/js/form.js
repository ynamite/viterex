...
class MASSIF_form {
    ...
    initForm = (form) => {
        ...
        document.dispatchEvent(
            new CustomEvent('forms_ready', { detail: form })
        );
    };
    ...
    processFormData = async (form) => {
        ...
        const endpoint = form.querySelector(
            'input[name="rex-api-call"]'
        )?.value;
    ...
    yformHandler = (form) => {
        ...
        const $responseForm = this.response[form.id].querySelector(
            '#' + form.id
        );
        if (!$responseForm) {
            const $successMessage =
                this.response[form.id].querySelector('.alert');
            if ($successMessage) {
    ...
    initCustomSelect = async (form) => {
        const module = await import(
            '/assets/theme/js/custom-select.js?v=0.0.1'
        );
